...
BEGIN
    ...
    WHILE InTargetDate<=InStopDate DO
        ...
        IF (MOD(YEAR(InTargetDate), 400) = 0) THEN
            SET daysInYear = 366;
        ELSE
            IF (MOD(YEAR(InTargetDate), 4) = 0
                    AND MOD(YEAR(InTargetDate), 100) != 0) THEN
                SET daysInYear = 366;
            ELSE
                SET daysInYear = 365;
            END IF;
        ...
        # Получение остатка  - остаток получаем за текущий
        # день не включительно 'date < InTargetDate'
        # тоесть получаем остаток словно измерение происходит
        # в 00:00 нового дня, как требуется по условию
        SELECT sum(if(operation = 0, 1, -1) * amount) as ostatok
        from movements, accounts
        where  date < InTargetDate
