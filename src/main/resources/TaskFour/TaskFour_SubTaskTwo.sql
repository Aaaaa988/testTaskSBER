...
SET @date = '2021-10-4';
SELECT IF( SUM(plan) - SUM(fact) > 0,
        SUM(plan) - SUM(fact),
        0
        ) AS another_dolg
FROM(
    SELECT fact_operat.amount AS fact,
           plan_operat.amount AS plan,
           COALESCE(fact_operat.date, plan_operat.date) AS date
    ...
    WHERE credit_contract2.num = @num_account
    ) AS summs
WHERE date <= @date;